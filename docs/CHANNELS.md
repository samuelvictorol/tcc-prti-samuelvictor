...
Revisado em **14 de agosto de 2026**. Este documento registra as limitações que a aplicação aplica ou deixa explícitas na interface. As políticas dos provedores continuam prevalecendo e devem ser revistas antes de cada implantação pública.
...
- A aplicação identifica o bot automaticamente com `getMe` e expõe somente `id`, nome e `@username`, nunca o token. Updates processados são persistidos conforme a política da conversa e também enviados aos administradores autenticados por Socket.IO.
- Uma mensagem privada comum torna a conversa conhecida, mas não equivale a consentimento para campanhas. Os comandos configurados `/notify-me` e `/verify-me` registram a autorização explícita e abrem o onboarding; `/login` gera acesso temporário ao Meu perfil, `/help` lista os comandos e `/stop` revoga.
...
- O envio rápido aceita somente um canal específico compatível. O modo global usa um conjunto ou templates escolhidos separadamente para um, dois ou três canais.
...
- Cada mudança recebida da Meta é persistida com classificação, resumo seguro, estado de processamento e payload criptografado. Campos de webhook que não representam uma conversa e mensagens com tipo ainda não suportado permanecem disponíveis na aba Webhook para diagnóstico.
...
- Um template criado no Notify Flow não cria nem aprova o modelo na Meta. Nome, idioma, ordem dos componentes, parâmetros nomeados ou posicionais e tipo de botão devem coincidir exatamente com a versão aprovada na conta que atende o número remetente. Mídias usam URL HTTPS pública assinada; botões de URL fixos não recebem sufixo dinâmico.
...
- `GMAIL_WEBHOOK_NOTIFICATION_EMAIL` é opcional e diferente do remetente. Quando preenchido, recebe um aviso administrativo para novas mensagens processadas por Telegram ou WhatsApp. Esse alerta é idempotente e auxiliar: uma falha de email não invalida o webhook nem bloqueia o canal de origem.
...
- A página pública não abre os documentos automaticamente. O visitante pode consultá-los pelo link fixo; quando tenta seguir por um canal sem aceite, uma dialog persistente apresenta Termos de Uso, Termos de Serviço e Política de Privacidade e mantém a ação bloqueada até a concordância. Documento ainda não publicado é sinalizado explicitamente e não causa bloqueio infinito.
