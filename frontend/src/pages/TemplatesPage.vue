...
export const META_LANGUAGE_OPTIONS = Object.freeze([
  ...
  Object.freeze({ label: 'Inglês — en', value: 'en' }),
])

export function normalizeMetaLanguageCode(value = '', fallback = 'pt_BR') {
  const languageCode = String(value || '').trim()
  if (languageCode === 'pt_BR' || languageCode === 'pt-BR' || languageCode === 'pt') return 'pt_BR'
  if (languageCode === 'en' || languageCode === 'en_US' || languageCode === 'en-US') return 'en'
  return fallback
}
...
export function hasPendingCloudMediaUpload(components = []) {
  return (components || []).some((component) => (
    (component.parameters || []).some((parameter) => parameter.uploading === true)
  ))
}
...
export function createCloudParameter(overrides = {}) {
  ...
  return {
    ...
    uploadFile: null,
    uploading: overrides.uploading === true,
  }
}
...
export function buildCustomWhatsAppCloudDefinition(input) {
  ...
  return {
    ...
    languageCode: normalizeMetaLanguageCode(input.languageCode),
...
export function buildCustomWhatsAppCloudPreviewPayload(input = {}) {
  ...
  return {
    ...
    template: {
      ...
      language: { code: normalizeMetaLanguageCode(input.languageCode) },
...
export function validateCustomWhatsAppCloudTemplate(input = {}) {
  ...
  if (languageCode && !META_LANGUAGE_OPTIONS.some((option) => option.value === languageCode)) return 'Escolha pt_BR ou en, conforme o idioma aprovado na Meta.'
...
const activeCloudMediaUploads = ref(0)
const cloudMediaUploadControllers = new Set()
...
const cloudMediaUploadInProgress = computed(() => (
  activeCloudMediaUploads.value > 0 || hasPendingCloudMediaUpload(form.cloudComponents)
))
...
function templateByRecordId(id) {
  if (!id) return null
  return templates.value.find((template) => String(recordId(template)) === String(id)) || null
}

function templateSetCloudMedia(components = []) {
  const header = components.find((component) => component.type === 'header')
  const parameter = header?.parameters?.find((item) => isCloudMediaParameter(item.type))
  const url = cloudParameterDisplayValue(parameter || {}).trim()
  if (!parameter || !url) return { type: '', url: '', filename: '' }
  return {
    type: parameter.type,
    url,
    filename: String(parameter.filename || parameter.uploadedFilename || '').trim() || 'Documento',
...
function templateSetChannelPreview(template = {}, channel = '') {
  if (channel === 'whatsapp_cloud') {
    ...
    const buttons = components.filter((component) => component.type === 'button')
    return {
      officialName: String(template.externalTemplateName || template.name || '').trim(),
      languageCode: normalizeMetaLanguageCode(template.languageCode || template.metadata?.language),
      header: previewCloudComponentText(header || {}),
      body: previewCloudComponentText(body || {}) || String(template.body || '').trim() || 'O modelo oficial não possui uma prévia textual cadastrada.',
      footer: previewCloudComponentText(footer || {}),
      buttons: buttons.map((button) => ({
        text: String(button.text || '').trim() || 'Ação',
        url: previewCloudButtonUrl(button),
      })),
      media: templateSetCloudMedia(components),
    }
  }
  ...
  if (channel === 'telegram') {
    ...
    return {
      body: telegramDefinitionBody(definition) || 'Este template não possui uma prévia textual.',
      media: ['photo', 'video'].includes(definition.kind)
        ? { type: definition.kind, url: String(definition.mediaUrl || '').trim(), filename: '' }
        : { type: '', url: '', filename: '' },
    }
  }

  return {
    subject: String(template.subject || '').trim() || 'Sem assunto',
    body: String(template.body || '').trim() || 'Este template não possui uma prévia textual.',
    html: template.html ? DOMPurify.sanitize(String(template.html), { USE_PROFILES: { html: true } }) : '',
    media: { type: '', url: '', filename: '' },
  }
}

const templateSetPreviewItems = computed(() => templateSetChannelOptions
  .map((channel) => {
    const template = templateByRecordId(templateSetForm.templateIds[channel.value])
    if (!template) return null
    return {
      ...channel,
      templateName: template.name || template.title || 'Template sem nome',
      preview: templateSetChannelPreview(template, channel.value),
    }
  })
  .filter(Boolean))
...
function removeCloudComponent(index) {
  if (cloudMediaUploadInProgress.value) return notifyCloudUploadBusy()
...
async function uploadCloudParameterMedia(parameter, selectedFile) {
  ...
  cloudMediaUploadControllers.add(controller)
  activeCloudMediaUploads.value += 1
  ...
  try {
    ...
    const uploaded = unwrap(await http.post('/media', body, { timeout: 600000, signal: controller.signal })) || {}
    ...
    cloudMediaUploadControllers.delete(controller)
    activeCloudMediaUploads.value = Math.max(0, activeCloudMediaUploads.value - 1)
  }
}

function notifyCloudUploadBusy() {
  $q.notify({
    type: 'info',
    message: 'Aguarde o upload terminar antes de alterar ou fechar o template.',
  })
}
...
function openEdit(template) {
  ...
  Object.assign(form, emptyForm(), {
    ...
    metadata: {
      ...
      language: normalizeMetaLanguageCode(template.languageCode || metadata.language || template.language),
    },
...
async function save() {
  if (cloudMediaUploadInProgress.value) {
    $q.notify({
      type: 'info',
      message: 'Aguarde a conclusão do upload antes de salvar o template.',
    })
    return
  }
  saving.value = true
...
onBeforeUnmount(() => {
  for (const controller of cloudMediaUploadControllers) controller.abort()
  cleanupPendingCloudMedia().catch(() => {})
})
      ...
      <q-card class="template-set-dialog">
        ...
        <q-form @submit.prevent="saveTemplateSet">
          <q-card-section class="template-set-dialog__content scroll">
            ...
            <section v-if="templateSetPreviewItems.length" class="template-set-preview-section">
              <header class="template-set-preview-heading">
                <div>
                  <span class="text-overline text-primary">Prévia do conjunto</span>
                  <h3>Confira o conteúdo selecionado</h3>
                  <p>Esta visualização reúne os templates que serão usados em cada canal.</p>
                </div>
                <q-badge outline color="primary" :label="`${templateSetPreviewItems.length} canal(is)`" />
              </header>

              <div class="template-set-preview-grid">
                <article
                  v-for="item in templateSetPreviewItems"
                  :key="item.value"
                  :class="['template-set-preview-card', `template-set-preview-card--${item.tone}`]"
                >
                  <header>
                    <span class="template-set-preview-channel-icon"><q-icon :name="item.icon" /></span>
                    <div>
                  ...
                  <div v-if="item.preview.officialName" class="template-set-preview-meta">
                  ...
                  <div v-if="item.preview.subject" class="template-set-preview-subject">
                    <span>Assunto</span>
                    <strong>{{ item.preview.subject }}</strong>
                  </div>
                  <div class="template-set-preview-message">
                    <span>Prévia da mensagem</span>
                    <strong v-if="item.preview.header" class="template-set-preview-component-text">{{ item.preview.header }}</strong>
                    <div v-if="item.preview.media?.url" class="template-set-preview-media">
                      <img
                        v-if="['photo', 'image'].includes(item.preview.media.type)"
                        :src="item.preview.media.url"
                        alt="Imagem do template"
                        loading="lazy"
                        referrerpolicy="no-referrer"
                      />
                      <video v-else-if="item.preview.media.type === 'video'" :src="item.preview.media.url" controls preload="metadata" />
                      <a v-else :href="item.preview.media.url" target="_blank" rel="noopener noreferrer">
                        <q-icon name="description" /> {{ item.preview.media.filename || 'Abrir documento' }}
                      </a>
                    </div>
                    <iframe
                      v-if="item.preview.html"
                      class="template-set-email-preview"
                      :srcdoc="item.preview.html"
                      :title="`Prévia HTML de ${item.templateName}`"
                      loading="lazy"
                      referrerpolicy="no-referrer"
                      sandbox
                    />
                    <p v-else>{{ item.preview.body }}</p>
                    <small v-if="item.preview.footer" class="template-set-preview-footer">{{ item.preview.footer }}</small>
                    <div v-if="item.preview.buttons?.length" class="template-set-preview-buttons">
    ...
    <q-dialog v-model="dialog" persistent :maximized="$q.screen.lt.md">
      <q-card class="template-dialog">
        <q-card-section class="row items-center q-px-lg q-py-md template-dialog__header">
          ...
          <q-space />
          <q-btn v-close-popup flat round dense icon="close" aria-label="Fechar" :disable="cloudMediaUploadInProgress">
            <q-tooltip v-if="cloudMediaUploadInProgress">Aguarde o upload terminar</q-tooltip>
          </q-btn>
        </q-card-section>
        <q-separator />
        <q-form @submit.prevent="save">
              ...
              <section class="builder-section">
                ...
                <div class="form-grid template-basics-grid">
                  ...
                  <q-select
                    ...
                    :disable="cloudMediaUploadInProgress"
          ...
          <q-card-actions align="right" class="q-pa-md q-px-lg template-dialog__footer">
            <q-btn v-close-popup flat no-caps label="Cancelar" :disable="cloudMediaUploadInProgress" />
            <q-btn
              type="submit"
              color="primary"
              unelevated
              ...
              :loading="saving"
              :disable="saving || cloudMediaUploadInProgress"
            >
              <q-tooltip v-if="cloudMediaUploadInProgress">Aguarde o upload terminar para salvar</q-tooltip>
            </q-btn>
          </q-card-actions>
        </q-form>
      </q-card>
...
.template-set-preview-section {
  margin-top: 22px;
  padding-top: 20px;
  border-top: 1px solid rgba(11, 92, 79, 0.12);
}

.template-set-preview-heading {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 16px;
...
.template-set-preview-heading h3,
.template-set-preview-heading p {
  margin: 0;
}
...
.template-set-preview-grid {
  display: grid;
  grid-template-columns: repeat(3, minmax(300px, 1fr));
  gap: 14px;
}

.template-set-preview-card {
  min-width: 0;
  overflow: hidden;
...
.template-set-preview-card > header {
  display: grid;
...
.template-set-preview-card > header div,
.template-set-preview-card > header strong,
.template-set-preview-card > header span {
  display: block;
  min-width: 0;
}
...
.template-set-preview-channel-icon {
...
.template-set-preview-card--telegram .template-set-preview-channel-icon {
  background: rgba(91, 184, 245, 0.18);
  color: #248bd6 !important;
}

.template-set-preview-card--gmail .template-set-preview-channel-icon {
  background: rgba(242, 130, 126, 0.17);
  color: #d9514e !important;
}

.template-set-preview-meta,
.template-set-preview-subject,
.template-set-preview-message {
  padding: 12px 14px;
...
.template-set-preview-meta > span,
.template-set-preview-subject > span,
.template-set-preview-message > span {
  display: block;
  color: #70817e;
  font-size: 0.7rem;
}
...
.template-set-preview-meta > strong,
.template-set-preview-message p {
  overflow-wrap: anywhere;
}

.template-set-preview-subject strong {
  display: block;
  margin-top: 3px;
}
...
.template-set-preview-component-text,
.template-set-preview-footer {
  display: block;
  margin-top: 7px;
  white-space: pre-wrap;
}
...
.template-set-preview-media {
  margin-top: 9px;
}

.template-set-preview-media img,
.template-set-preview-media video {
  display: block;
  width: 100%;
  max-height: 230px;
  object-fit: contain;
...
.template-set-email-preview {
  display: block;
  width: 100%;
  height: 430px;
  margin-top: 9px;
  border: 1px solid rgba(14, 89, 78, 0.12);
  border-radius: 12px;
  background: #fff;
}

.template-set-preview-buttons {
  display: grid;
  gap: 6px;
  margin-top: 10px;
}

.template-set-preview-buttons span {
  display: flex;
  align-items: center;
  justify-content: center;
  gap: 6px;
  padding: 8px;
  border-top: 1px solid rgba(14, 89, 78, 0.1);
  color: #138675;
  font-weight: 700;
}
