...
const publicBrandLogo = computed(() => inviteIcon.value || (brandLogoFailed.value ? '' : branding.logoUrl) || '')

function onPublicLogoError() {
  if (inviteIcon.value) {
    inviteIconFailed.value = true
    return
  }
  ...
  <main class="public-page" :style="{ background: gradient }">
    ...
    <q-card v-else flat class="public-card glass-card">
      <div class="public-logo">
        <img v-if="publicBrandLogo" :src="publicBrandLogo" :alt="`Ícone de ${invite.title}`" referrerpolicy="no-referrer" @error="onPublicLogoError" />
