...
const removingWebhookNotificationEmail = ref(false)
...
const settings = reactive({
  ...
  email: { user: '', appPassword: '', from: '', fromName: '', webhookNotificationEmail: '' },
  usefulLinks: [],
})
...
const webhookNotificationEmailConfigured = computed(() => Boolean(
  savedCredentialPreviews.email?.webhookNotificationEmail,
))
...
function removeWebhookNotificationEmail() {
  $q.dialog({
    title: 'Desativar avisos por email?',
    message: 'As mensagens continuarão chegando normalmente nos chats e webhooks, mas não serão mais encaminhadas ao email configurado.',
    cancel: { label: 'Cancelar', flat: true },
    ok: { label: 'Desativar avisos', color: 'negative', unelevated: true },
    persistent: true,
  }).onOk(async () => {
    removingWebhookNotificationEmail.value = true
    try {
      const result = unwrap(await http.delete('/settings/GMAIL_WEBHOOK_NOTIFICATION_EMAIL')) || {}
      if (result.envFallback) {
        applySettings(await app.fetchSettings())
        $q.notify({
          type: 'warning',
          message: 'A configuração salva foi removida, mas o destinatário definido no ambiente continua ativo.',
          caption: 'Remova GMAIL_WEBHOOK_NOTIFICATION_EMAIL do ambiente do servidor para desativar os avisos.',
        })
      } else {
        settings.email.webhookNotificationEmail = ''
        savedCredentialPreviews.email.webhookNotificationEmail = ''
        delete revealedCredentialBaselines.email.webhookNotificationEmail
        $q.notify({ type: 'positive', message: 'Avisos de novas mensagens por email desativados.' })
      }
    } catch (error) {
      $q.notify({
        type: 'negative',
        message: errorMessage(error, 'Não foi possível desativar os avisos por email.'),
      })
    } finally {
      removingWebhookNotificationEmail.value = false
    }
  })
}
      ...
      <q-card flat class="glass-card section-card">
        ...
        >
          <div class="form-grid q-pa-md">
            ...
            <q-separator class="full-span q-my-sm" />
            <div class="full-span webhook-email-heading">
              <div>
                <strong>Email Receber Notificação</strong>
                <span>Encaminhamento opcional das novas mensagens recebidas pelo WhatsApp e Telegram.</span>
              </div>
              <ContextHelp
                title="Avisos de novas mensagens"
                tooltip="Entenda o encaminhamento por email"
                :text="[
                  'Quando preenchido, este endereço recebe um aviso enviado pela conta Gmail configurada sempre que uma nova mensagem chega pelos webhooks do WhatsApp ou Telegram.',
                  'O Gmail precisa estar completamente configurado. O destinatário pode ser igual ou diferente da conta remetente, e uma falha no aviso não interrompe o recebimento da mensagem original.',
                ]"
              />
            </div>
            <q-input
              v-model="settings.email.webhookNotificationEmail"
              class="full-span"
              outlined
              type="email"
              :readonly="credentialFieldIsMasked('email', 'webhookNotificationEmail')"
              label="Email para receber avisos (opcional)"
              hint="Assunto: [TELEGRAM] Nome: mensagem ou [WHATSAPP] Nome: mensagem"
            />
            <div class="full-span channel-actions">
              <q-btn outline color="primary" no-caps icon="save" label="Salvar Gmail" :loading="savingChannel.email" @click="saveChannel('email')" />
              <q-btn
                v-if="webhookNotificationEmailConfigured"
                flat
                color="negative"
                no-caps
                icon="notifications_off"
                label="Desativar avisos"
                :loading="removingWebhookNotificationEmail"
                @click="removeWebhookNotificationEmail"
              />
...
.webhook-email-heading > div:first-child {
  display: grid;
  gap: 3px;
}

.webhook-email-heading span {
  color: #637875;
  font-size: 0.78rem;
  line-height: 1.45;
}
