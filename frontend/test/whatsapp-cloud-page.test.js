...
describe('histórico persistente de webhooks da Meta', () => {
  ...
  it('organiza eventos, contatos e falhas na aba webhook com 10 registros por página', () => {
    ...
    const webhookTabIndex = source.indexOf('<div v-show="activeTab === \'webhook\'" class="cloud-webhook-content">')
    const eventsIndex = source.indexOf('<h2 class="section-title">Eventos do webhook</h2>', webhookTabIndex)
    const contactsIndex = source.indexOf('<h2 class="section-title">Contatos recebidos pelo webhook</h2>', webhookTabIndex)
    const issuesIndex = source.indexOf('<h2 class="section-title">Contatos ignorados ou com falha</h2>', webhookTabIndex)
    expect(eventsIndex).toBeGreaterThan(webhookTabIndex)
    expect(eventsIndex).toBeLessThan(contactsIndex)
    expect(contactsIndex).toBeLessThan(issuesIndex)
    expect(source).not.toMatch(/\.cloud-webhook-content \.webhook-events-panel\s*\{[\s\S]*?order:/)
