...
describe('templates oficiais do WhatsApp Cloud', () => {
  it('bloqueia o salvamento enquanto qualquer mídia ainda está em upload', () => {
    expect(hasPendingCloudMediaUpload([
      createCloudComponent({ type: 'header', parameters: [{ type: 'image', uploading: true }] }),
    ])).toBe(true)
    expect(hasPendingCloudMediaUpload([
      createCloudComponent({ type: 'header', parameters: [{ type: 'image', uploading: false }] }),
    ])).toBe(false)
  })
  ...
  it('oferece somente os idiomas suportados no formulário amigável', () => {
    expect(META_LANGUAGE_OPTIONS.map((option) => option.value)).toEqual(['pt_BR', 'en'])
  })

  it('normaliza registros antigos antes de montar o payload oficial', () => {
    expect(normalizeMetaLanguageCode('en_US')).toBe('en')
    expect(normalizeMetaLanguageCode('en-US')).toBe('en')
    expect(normalizeMetaLanguageCode('pt-BR')).toBe('pt_BR')
    expect(buildCustomWhatsAppCloudPreviewPayload({
      templateName: 'modelo_legado',
      languageCode: 'en_US',
      components: [],
    }).template.language.code).toBe('en')
  })
  ...
  it('oferece upload multipart, prévia visual e inspeção do payload sem JSON manual', () => {
    ...
    expect(source).toContain("http.post('/media', body, { timeout: 600000, signal: controller.signal })")
    expect(source).toContain(':disable="saving || cloudMediaUploadInProgress"')
    expect(source).toContain('Aguarde o upload terminar para salvar')
    expect(source).toContain('if (!id || !pendingCloudMediaAssetIds.has(id)) return')
