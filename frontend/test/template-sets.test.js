...
describe('conjuntos de templates', () => {
  ...
  it('expõe CRUD, busca por convite e vínculo a partir da row de templates', () => {
    ...
    expect(source).toContain('templateSetPreviewItems')
    expect(source).toContain('Prévia do conjunto')
    expect(source).toContain('class="template-set-email-preview"')
    expect(source).toContain(':srcdoc="item.preview.html"')
    expect(source).toContain('referrerpolicy="no-referrer"')
    expect(source).toContain('max-width: 1320px !important')
  })
})
