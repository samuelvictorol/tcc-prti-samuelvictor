...
| Rota | Função |
|---|---|
| `/login` | escolha entre acesso administrativo e acesso do contato ao Meu perfil |
| `/invite/:slug` | convite público, links por canal, QR da URL completa e acesso aos documentos legais |
...
| `/telegram` | disparos, conversas conhecidas, grupos, webhook e mensagens realtime |
| `/chats` | redirecionamento de compatibilidade para a aba Conversas do WhatsApp Cloud |
| `/whatsapp-cloud` | disparos, conversas oficiais em tempo real, eventos persistidos do webhook, contatos e templates |
...
| `/whitelabel` | identidade visual, uso do banco, exportação, limpeza e auditoria de armazenamento |
| `/help` | guias de operação e comandos disponíveis por canal |
...
- Após trocar um link opaco de uso único, o profile token separado fica em `localStorage` com a expiração.
...
| `services/template-sets.js` | conjuntos multicanal e vínculo opcional com convite |
| `services/storage-admin.js` | uso, exportação, limpeza confirmada e auditoria do banco |
| `services/whitelabel.js` | carregamento e atualização da marca aplicada ao produto |
| `services/useful-links.js` | atalhos administrativos exibidos na navegação |
| `services/sounds.js` | áudio deduplicado para mensagens e avisos em tempo real |
...
| `whatsapp_cloud:webhook`, `whatsapp_cloud:webhook_event` | resumo realtime e atualização incremental dos eventos Meta persistidos |
...
- **WhatsApp Cloud:** nome oficial, idioma, mídia, corpo e botões construídos por formulário; suporta valor fixo ou solicitado no disparo, preview visual e inspeção do payload sem exigir JSON manual.
...
- **Conjunto:** seleção de até um template por canal, convite opcional e preview multicanal antes de salvar ou enfileirar.
- **Termos:** visualização sanitizada em dialog persistente e rolável. A página pública não abre a dialog automaticamente; ao tentar seguir por um canal sem aceite, ela solicita a concordância antes de continuar.
- **Whitelabel:** nome, título, logotipo, paleta e rodapé com aplicação imediata após salvar.

HTML é permitido apenas para email e documentos legais; nunca é renderizado sem sanitização.

Mídias carregadas para templates ou para a identidade Whitelabel exibem estado de upload e bloqueiam o salvamento enquanto a operação está pendente. A UI envia o asset ao backend, recebe uma URL assinada e solicita o descarte quando o rascunho é abandonado; a API continua sendo responsável por validar referências antes da remoção definitiva.

## Administração do armazenamento

A página **Whitelabel** apresenta o consumo geral e por collection. O administrador pode exportar o escopo permitido, incluir mídias em ZIP, limpar uma collection elegível ou iniciar a limpeza geral. Operações destrutivas exigem confirmação explícita e mostram o progresso e o resultado no console de auditoria. A interface não presume que toda collection possa ser removida: as políticas vêm do backend.
