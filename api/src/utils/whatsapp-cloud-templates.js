...
const CUSTOM_TEMPLATE_LANGUAGE_CODES = Object.freeze(['pt_BR', 'en']);

function customTemplateLanguageCode(value, fallback = '', options = {}) {
  const rawLanguageCode = String(value || fallback).trim();
  if (options.allowSystemEnUs === true && rawLanguageCode === 'en_US') return 'en_US';
  // Versoes antigas do formulario ofereciam en_US. Templates personalizados
  // em ingles generico aprovados pela Meta usam `en`; normalize o legado no
  // limite da API sem alterar os presets oficiais de teste, que seguem en_US.
  const languageCode = ['en_US', 'en-US'].includes(rawLanguageCode) ? 'en' : rawLanguageCode;
  if (!CUSTOM_TEMPLATE_LANGUAGE_CODES.includes(languageCode)) {
    templateError(
      'Idioma de template personalizado deve ser pt_BR ou en, exatamente como aprovado na Meta',
      { field: 'languageCode', allowed: CUSTOM_TEMPLATE_LANGUAGE_CODES }
    );
  }
  return languageCode;
}
...
function buildCustomTemplateMessage(customTemplate) {
  ...
  const languageCode = customTemplateLanguageCode(customTemplate?.languageCode, '', {
    allowSystemEnUs: customTemplate?.systemManaged === true
  });
...
function normalizeOfficialTemplateDefinition(input, options = {}) {
  ...
  if (inferredPreset === 'custom') {
    ...
    const languageCode = customTemplateLanguageCode(input.languageCode, 'pt_BR', {
      allowSystemEnUs: options.allowSystemEnUs === true
    });
    if (!/^[a-z0-9_]{1,512}$/.test(name)) templateError('Nome oficial Meta invalido', { field: 'externalTemplateName' });
