...
const RETAINED_MEDIA_GRACE_MS = 24 * 60 * 60 * 1000;
const MEDIA_DELETE_CLAIM_MS = 5 * 60 * 1000;
...
function assetIdFromPublicUrl(value) {
  const input = String(value || '').trim();
  if (!input) return null;
  let pathname;
  try {
    pathname = new URL(input, 'https://notify-flow.invalid').pathname;
  } catch (_error) {
    return null;
  }
  const prefix = String(env.apiPrefix || '/api').replace(/\/$/, '') + '/media/';
  const index = pathname.lastIndexOf(prefix);
  if (index < 0) return null;
  const encodedToken = pathname.slice(index + prefix.length).split('/', 1)[0];
  if (!encodedToken) return null;
  try {
    return parsePublicToken(decodeURIComponent(encodedToken));
  } catch (_error) {
    return null;
  }
}
...
function normalizePurpose(value) {
  ...
  if (!['template', 'dispatch', 'branding'].includes(purpose)) {
    throw new ApiError(
      422,
      'Finalidade do upload invalida',
      { field: 'purpose', allowed: ['template', 'dispatch', 'branding'] },
...
async function upload(file, requestedMediaType, actorId, options = {}) {
  ...
  if (purpose === 'branding' && validated.mediaType !== 'image') {
    throw new ApiError(
      422,
      'A identidade visual aceita somente imagem JPG ou PNG',
      { field: 'file', mediaType: validated.mediaType },
      'BRANDING_MEDIA_TYPE_INVALID'
    );
  }
...
async function open(token, rangeHeader, options = {}) {
  ...
  const query = {
    ...
    'metadata.status': { $nin: ['orphaned', 'deleting'] },
    $or: [
      { 'metadata.expiresAt': { $exists: false } },
      { 'metadata.expiresAt': { $gt: new Date() } }
    ]
  };
  let file = await bucket.find(query).next();
  // Uma atualizacao concorrente pode tombstonar um asset logo depois de outra
  // gravacao passar a referencia-lo. Antes de devolver 404, restaure apenas
  // quando a referencia persistida realmente existir.
  if (!file && await restoreRetained(normalizedId, options)) {
    file = await bucket.find(query).next();
  }
...
async function retain(assetIds, options = {}) {
  ...
  const purpose = options.purpose ? normalizePurpose(options.purpose) : null;
  const set = { 'metadata.status': 'retained' };
  if (purpose) set['metadata.purpose'] = purpose;
  const result = await db.collection(BUCKET_NAME + '.files').updateMany(
    {
      _id: { $in: ids },
      'metadata.scope': 'whatsapp_template',
      $or: [
        { 'metadata.status': { $in: ['pending', 'retained', 'orphaned', 'deleting'] } },
...
async function restoreRetained(assetId, options = {}) {
  const id = normalizeAssetIds([assetId])[0];
  if (!id) return false;
  const db = options.db || mongoose.connection.db;
  if (!db?.collection) return false;
  const referencedByTemplate = typeof options.isTemplateReferenced === 'function'
    ? await options.isTemplateReferenced(id)
    : await require('./templates.manager').isTemplateMediaReferenced(id);
  let referencedByBranding = false;
  try {
    referencedByBranding = typeof options.isBrandingReferenced === 'function'
      ? await options.isBrandingReferenced(id)
      : await require('./settings.manager').isWhitelabelMediaReferenced(id);
  } catch (_error) {
    referencedByBranding = false;
  }
  if (!referencedByTemplate && !referencedByBranding) return false;
  await retain([id], {
    db,
    purpose: referencedByBranding && !referencedByTemplate ? 'branding' : 'template'
  });
  return true;
}
...
async function removeRetained(assetIds, options = {}) {
  const ids = normalizeAssetIds(assetIds).map((id) => new mongoose.Types.ObjectId(id));
  if (!ids.length) return { removed: 0, scheduled: 0 };
  const db = options.db || mongoose.connection.db;
  if (!db?.collection) throw new ApiError(503, 'Armazenamento de midia indisponivel', null, 'TEMPLATE_MEDIA_STORAGE_UNAVAILABLE');
  const now = options.now || new Date();
  const graceMs = Math.max(60_000, Number(options.graceMs) || RETAINED_MEDIA_GRACE_MS);
  const result = await db.collection(BUCKET_NAME + '.files').updateMany(
    {
      _id: { $in: ids },
      'metadata.scope': 'whatsapp_template',
      'metadata.status': { $in: ['retained', 'orphaned'] }
    },
    {
      $set: {
        'metadata.status': 'orphaned',
        'metadata.tombstonedAt': now,
        'metadata.expiresAt': new Date(now.getTime() + graceMs)
      }
    }
  );
  return { removed: 0, scheduled: Number(result.modifiedCount || result.matchedCount || 0) };
}

async function reconcileRetainedPurpose(purposeValue, referencedAssetIds, options = {}) {
  const purpose = normalizePurpose(purposeValue);
  const referenced = new Set(normalizeAssetIds(referencedAssetIds));
  const db = options.db || mongoose.connection.db;
  if (!db?.collection) throw new ApiError(503, 'Armazenamento de midia indisponivel', null, 'TEMPLATE_MEDIA_STORAGE_UNAVAILABLE');
  ...
  const detached = files.map((file) => String(file._id)).filter((id) => !referenced.has(id));
  return removeRetained(detached, { ...options, db });
}
...
async function cleanupExpired(options = {}) {
  ...
  for (const file of expired) {
    try {
      if (['orphaned', 'deleting'].includes(file.metadata?.status)) {
        // A verificacao ocorre somente depois da carencia e imediatamente antes
        // da exclusao. Um create/clone/update concorrente pode restaurar o asset
        // por retain() durante toda a janela de seguranca.
        if (typeof options.isReferenced !== 'function') {
          skipped += 1;
          continue;
        }
        if (await options.isReferenced(String(file._id))) {
          await retain([String(file._id)], { db });
          restored += 1;
          continue;
        }
        const claim = await db.collection(BUCKET_NAME + '.files').findOneAndUpdate({
          _id: file._id,
          'metadata.scope': 'whatsapp_template',
          'metadata.status': file.metadata.status,
          'metadata.expiresAt': { $lte: options.now || new Date() }
        }, {
          $set: {
            'metadata.status': 'deleting',
            'metadata.deleteClaimedAt': options.now || new Date(),
            'metadata.expiresAt': new Date((options.now || new Date()).getTime() + MEDIA_DELETE_CLAIM_MS)
          }
        ...
        // Segunda leitura fecha a janela entre a reconciliacao inicial e a
        // confirmacao do tombstone expirado.
        if (await options.isReferenced(String(file._id))) {
          if (!await restoreRetained(String(file._id), { db })) {
            skipped += 1;
            continue;
          }
          restored += 1;
  ...
  return { removed, restored, skipped, scanned: expired.length };
}
...
module.exports = {
  ...
  discardPending,
  removeRetained,
  reconcileRetainedPurpose,
  cleanupExpired,
  MEDIA_RULES,
  BUCKET_NAME,
  TEMPLATE_DRAFT_TTL_MS,
  DISPATCH_MEDIA_TTL_MS,
  RETAINED_MEDIA_GRACE_MS,
  MEDIA_DELETE_CLAIM_MS
};
