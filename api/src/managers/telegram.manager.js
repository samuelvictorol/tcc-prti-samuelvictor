...
const inboundWebhookEmail = require('../services/inbound-webhook-email.service');
const {
...
async function webhook(update, providedSecret) {
  ...
  if (message?.chat) {
    ...
    if (chat.type === 'private') {
      ...
      if (!recordedInbound?.discardedByRemoval) {
        await inboundWebhookEmail.notify({
          channel: 'telegram',
          externalId: String(chat.id),
          providerMessageId: message.message_id,
          displayName: displayName(message.from || chat, String(chat.id)),
          message: permissionInvocation.inviteAttributionMarker
            ? permissionInvocation.command
            : safeInboundText || `[${messageType(message)}]`
        });
      }
      ...
      if (group && !recordedInbound?.discardedByRemoval) {
        await inboundWebhookEmail.notify({
          channel: 'telegram',
          externalId: String(chat.id),
          providerMessageId: message.message_id,
          displayName: `${chat.title || String(chat.id)}${message.from ? ` - ${displayName(message.from, '')}` : ''}`,
          message: message.text || message.caption || `[${messageType(message)}]`
        });
      }
