...
function templateMediaAssetIds(template = {}) {
  ...
  const visit = (value, key = '') => {
    ...
    if (key === 'mediaAssetId') values.push(value);
    if (typeof value === 'string') {
      const assetId = templateMediaManager.assetIdFromPublicUrl(value);
      if (assetId) values.push(assetId);
    }
  };
  visit(template.payload || {});
  return templateMediaManager.normalizeAssetIds(values);
}

async function referencedTemplateMediaAssetIds(assetIds) {
  const ids = templateMediaManager.normalizeAssetIds(assetIds);
  if (!ids.length) return new Set();
  const [referenced, payloadTemplates] = await Promise.all([
    Template.distinct(
      'payload.builder.components.parameters.mediaAssetId',
      { 'payload.builder.components.parameters.mediaAssetId': { $in: ids } }
    ),
    // Versões antigas podiam persistir somente a URL pública assinada. A
    // leitura limitada ao payload também reconhece links cujo host mudou.
    Template.find({ channel: 'whatsapp_cloud', payload: { $exists: true } }).select('payload').lean()
  ]);
  const referencedSet = new Set(referenced.map(String));
  for (const template of payloadTemplates) {
    for (const assetId of templateMediaAssetIds(template)) referencedSet.add(assetId);
  }
  return referencedSet;
}

async function isTemplateMediaReferenced(assetId) {
  const id = templateMediaManager.normalizeAssetIds([assetId])[0];
  if (!id) return false;
  return (await referencedTemplateMediaAssetIds([id])).has(id);
}

async function cleanupDetachedTemplateMedia(assetIds) {
  const ids = templateMediaManager.normalizeAssetIds(assetIds);
  if (!ids.length) return { removed: 0, scheduled: 0 };
  const referencedSet = await referencedTemplateMediaAssetIds(ids);
...
function normalize(input) {
  if (input.channel === 'whatsapp_cloud') {
    ...
    const normalized = normalizeOfficialTemplateDefinition(input, {
      ...
      allowSystemEnUs: internalSystemTemplate
    });
