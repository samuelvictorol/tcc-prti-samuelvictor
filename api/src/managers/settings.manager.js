...
const { usefulLinksSchema, whitelabelSchema } = require('../dtos/settings.dto');

const WHITELABEL_KEY = 'WHITELABEL_CONFIGURATION';
const DEFAULT_WHITELABEL = Object.freeze({
  ...
  links: Object.freeze({
    instagram: 'https://www.instagram.com/aitosoftwares/',
    website: 'https://aitosoftwares.com/'
  }),
  footer: Object.freeze({ text: 'Powered by @aitosoftwares' })
});
...
const DEFINITIONS = Object.freeze({
  ...
  GMAIL_WEBHOOK_NOTIFICATION_EMAIL: { sensitive: false, channel: 'email' },
...
const CHANNEL_REVEAL_FIELDS = Object.freeze({
  ...
  email: Object.freeze({
    ...
    webhookNotificationEmail: 'GMAIL_WEBHOOK_NOTIFICATION_EMAIL'
  })
});
...
function cloneWhitelabel(value = DEFAULT_WHITELABEL) {
  return JSON.parse(JSON.stringify(value));
}

function mergeWhitelabel(current, update = {}) {
  return {
    identity: { ...current.identity, ...(update.identity || {}) },
    colors: { ...current.colors, ...(update.colors || {}) },
    links: { ...current.links, ...(update.links || {}) },
    footer: { ...current.footer, ...(update.footer || {}) }
  };
}

function parseWhitelabel(value, options = {}) {
  let candidate = value;
  if (typeof candidate === 'string') {
    try {
      candidate = JSON.parse(candidate);
    } catch (_error) {
      if (options.fallback) return cloneWhitelabel();
      throw new ApiError(422, 'A configuracao whitelabel possui formato invalido', null, 'WHITELABEL_INVALID');
    }
  }
  const parsed = whitelabelSchema.safeParse(candidate);
  if (parsed.success) return parsed.data;
  if (options.fallback) return cloneWhitelabel();
  throw new ApiError(
    422,
    'Revise a configuracao whitelabel informada',
    { fieldErrors: parsed.error.flatten().fieldErrors },
    'WHITELABEL_INVALID'
  );
}
...
async function getWhitelabel() {
  const stored = await Setting.findOne({ key: WHITELABEL_KEY }).select('+valueEncrypted').lean();
  if (!stored) return cloneWhitelabel();
  try {
    return parseWhitelabel(decrypt(stored.valueEncrypted), { fallback: true });
  } catch (_error) {
    return cloneWhitelabel();
  }
}

async function setWhitelabel(input, actorId) {
  const current = await getWhitelabel();
  const configuration = parseWhitelabel(mergeWhitelabel(current, input));
  const previousAssetId = templateMediaManager.assetIdFromPublicUrl(current.identity.logoUrl);
  const nextAssetId = templateMediaManager.assetIdFromPublicUrl(configuration.identity.logoUrl);
  if (nextAssetId) await templateMediaManager.retain([nextAssetId], { purpose: 'branding' });
  try {
    await Setting.updateOne(
      { key: WHITELABEL_KEY },
      {
        $set: {
          valueEncrypted: encrypt(JSON.stringify(configuration)),
          sensitive: false,
          updatedBy: actorId
        }
      },
    ...
    if (nextAssetId && nextAssetId !== previousAssetId) {
      await templateMediaManager.removeRetained([nextAssetId]).catch(() => {});
    }
    throw error;
  }
  if (previousAssetId && previousAssetId !== nextAssetId) {
    // A URL deixa de responder imediatamente; os bytes permanecem durante a
    // carencia e podem ser restaurados por uma atualizacao concorrente.
    const latest = await getWhitelabel();
    if (templateMediaManager.assetIdFromPublicUrl(latest.identity.logoUrl) !== previousAssetId) {
      await templateMediaManager.removeRetained([previousAssetId]).catch(() => {});
    }
  }
  const latestBranding = await getWhitelabel();
  const latestAssetId = templateMediaManager.assetIdFromPublicUrl(latestBranding.identity.logoUrl);
  await templateMediaManager.reconcileRetainedPurpose('branding', latestAssetId ? [latestAssetId] : [])
    .catch((error) => console.error('[branding-media reconcile]', error.message));
  // Outra atualizacao pode vencer entre o update e a reconciliacao. Retorne a
  // configuracao que realmente ficou persistida, mantendo resposta e refs de
  // branding coerentes com o estado observado pelo proximo GET.
  return latestBranding;
}

async function isWhitelabelMediaReferenced(assetId) {
  const normalized = templateMediaManager.normalizeAssetIds([assetId])[0];
  if (!normalized) return false;
  const branding = await getWhitelabel();
  return templateMediaManager.assetIdFromPublicUrl(branding.identity.logoUrl) === normalized;
}
...
function assertWritableValue(key, value, config) {
  ...
  if (key === 'GMAIL_WEBHOOK_NOTIFICATION_EMAIL') {
    const normalized = String(value || '').trim().toLowerCase();
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalized) || normalized.length > 254) {
      throw new ApiError(
        422,
        'Informe um email valido para receber notificacoes dos webhooks',
        { key },
        'WEBHOOK_NOTIFICATION_EMAIL_INVALID'
      );
    }
  }
}
...
async function setValue(key, value, actorId, options = {}) {
  ...
  if (normalized === 'GMAIL_WEBHOOK_NOTIFICATION_EMAIL'
    && !options.skipChannelDependencyValidation
    && !await channelConfigured('email')) {
    throw new ApiError(
      422,
      'Configure a conta Gmail, a senha de app e o email remetente antes de ativar os avisos de webhook',
      { field: 'email.webhookNotificationEmail' },
      'WEBHOOK_NOTIFICATION_REQUIRES_GMAIL'
    );
  }
...
async function setBulk(input, actorId) {
  const mapping = {
    ...
    'email.webhookNotificationEmail': 'GMAIL_WEBHOOK_NOTIFICATION_EMAIL',
  ...
  const pendingByKey = Object.fromEntries(pending.map((item) => [item.key, item.value]));
  if (pendingByKey.GMAIL_WEBHOOK_NOTIFICATION_EMAIL) {
    const gmailValues = await Promise.all(REQUIRED.email.map(async (key) => (
      pendingByKey[key] || await getValue(key)
    )));
    if (!gmailValues.every((value) => Boolean(unwrapConfiguredValue(value)))) {
      throw new ApiError(
        422,
        'Configure a conta Gmail, a senha de app e o email remetente antes de ativar os avisos de webhook',
        { field: 'email.webhookNotificationEmail' },
  ...
  for (const item of pending) {
    updated.push(await setValue(item.key, item.value, actorId, {
      skipPermissionValidation: true,
      skipChannelDependencyValidation: item.key === 'GMAIL_WEBHOOK_NOTIFICATION_EMAIL'
    }));
  }
...
async function getStructured() {
  ...
  return {
    ...
    email: {
      ...
      webhookNotificationEmail: values.GMAIL_WEBHOOK_NOTIFICATION_EMAIL?.value || null,
...
module.exports = {
  ...
  DEFAULT_WHITELABEL, WHITELABEL_KEY, parseWhitelabel, getWhitelabel, setWhitelabel,
  isWhitelabelMediaReferenced
};
