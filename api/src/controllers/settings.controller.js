...
async function whitelabel(_req, res) {
  const branding = await settingsManager.getWhitelabel();
  res.set('Cache-Control', 'no-cache, max-age=0');
  res.json({ success: true, data: { branding } });
}

async function updateWhitelabel(req, res) {
  const branding = await settingsManager.setWhitelabel(req.validated.body.branding, req.admin.id);
  await logsManager.create({
    level: 'info',
    channel: 'system',
    action: 'settings.whitelabel_updated',
    message: 'Administrador atualizou a identidade visual da aplicacao',
    actor: req.admin.id,
    requestId: req.id
  });
  res.json({ success: true, data: { branding } });
...
module.exports = { list, statuses, whitelabel, updateWhitelabel, reveal, update, updateBulk, remove };
