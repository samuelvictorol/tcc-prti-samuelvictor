...
const { isSafeExternalHttpUrl, isSafePublicHttpsUrl } = require('../utils/urls');
...
const whitelabelColor = z.string()
  .trim()
  .regex(/^#[0-9a-f]{6}$/i, 'Informe uma cor hexadecimal no formato #RRGGBB')
  .transform((value) => value.toUpperCase());

const whitelabelText = (maximum) => z.string()
  .trim()
  .min(1)
  .max(maximum)
  .refine((value) => !hasDisallowedControlCharacters(value), 'O texto nao pode conter caracteres de controle');

const whitelabelHttpsUrl = z.string()
  .trim()
  .max(2048)
  .refine(isSafePublicHttpsUrl, 'Informe uma URL HTTPS publica e segura')
  .transform((value) => new URL(value).toString());

const optionalWhitelabelUrl = z.preprocess(
  (value) => value === null || typeof value === 'string' && value.trim() === '' ? null : value,
  z.union([whitelabelHttpsUrl, z.null()])
);

const whitelabelSchema = z.object({
  identity: z.object({
    name: whitelabelText(80),
    title: whitelabelText(120),
    logoUrl: optionalWhitelabelUrl
  }).strict(),
  colors: z.object({
    primary: whitelabelColor,
    secondary: whitelabelColor,
    accent: whitelabelColor,
    background: whitelabelColor
  }).strict(),
  links: z.object({
    instagram: optionalWhitelabelUrl,
    website: optionalWhitelabelUrl
  }).strict(),
  footer: z.object({
    text: whitelabelText(120)
  }).strict()
}).strict();
...
const whitelabelUpdateSchema = z.object({
  body: z.object({
    branding: z.object({
      identity: z.object({
        name: whitelabelText(80).optional(),
        title: whitelabelText(120).optional(),
        logoUrl: optionalWhitelabelUrl.optional()
      }).strict().optional(),
      colors: z.object({
        primary: whitelabelColor.optional(),
        secondary: whitelabelColor.optional(),
        accent: whitelabelColor.optional(),
        background: whitelabelColor.optional()
      }).strict().optional(),
      links: z.object({
        instagram: optionalWhitelabelUrl.optional(),
        website: optionalWhitelabelUrl.optional()
      }).strict().optional(),
      footer: z.object({
        text: whitelabelText(120).optional()
      }).strict().optional()
    }).strict().refine((branding) => Object.keys(branding).length > 0, 'Informe ao menos uma configuracao')
  }).strict()
});
...
const bulkSettingsSchema = z.object({
  body: z.object({
    ...
    email: z.object({
      ...
      webhookNotificationEmail: optionalField(z.string().email().max(254))
    }).optional(),
