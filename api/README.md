...
| Settings e Whitelabel | `GET /settings`, `/settings/status`, `/settings/whitelabel`; atualização por `PUT` e revelação temporária controlada por canal |
...
| Templates | CRUD e clonagem lógica em `/templates` |
| Conjuntos de templates | CRUD, busca e vínculo com convite em `/template-sets` |
| Mídia de templates | upload autenticado `POST /media` (`multipart/form-data`, campo `file`), descarte `DELETE /media/:id` e leitura pública por URL assinada `GET /media/:token` |
...
| Eventos Meta | lista e detalhe persistidos em `/whatsapp-cloud/webhook-events` |
| Gmail | status/envio em `/email`; `/gmail` é alias; alerta opcional de inbound usa `GMAIL_WEBHOOK_NOTIFICATION_EMAIL` |
| Armazenamento | uso, collections, exportação, limpeza confirmada e auditoria em `/system/storage-*` |

Uploads de cabeçalho para templates oficiais e logotipos Whitelabel são armazenados como binário no GridFS (`templateMedia.files`/`templateMedia.chunks`), nunca como base64 dentro do documento principal. A resposta contém `id`, `url`, `mimeType`, `mediaType`, `filename` e `size`; o builder salva a URL e o identificador associado ao parâmetro de imagem, vídeo ou documento. Rascunhos não utilizados expiram em 24 horas, mídias de um disparo direto expiram em 7 dias e os arquivos vinculados permanecem enquanto houver referência. Ao substituir, desvincular ou excluir um template ou logotipo, a API remove o asset somente depois de confirmar que nenhuma referência válida depende dele. A URL pública é uma capability assinada estável que a Meta pode baixar sem token administrativo. Em produção, `PUBLIC_APP_URL` (ou `MEDIA_PUBLIC_BASE_URL`) precisa apontar para o domínio HTTPS público.
...
## Whitelabel e administração do armazenamento

`GET/PUT /api/settings/whitelabel` consulta ou atualiza nome, título, logotipo, paleta e texto do rodapé. O frontend aplica a identidade visual sem alterar os contratos dos canais. Quando o logotipo é enviado ao próprio Notify Flow, ele usa o mesmo armazenamento assinado de mídia e o ciclo de referências descrito acima.

As rotas autenticadas de `/api/system` oferecem:

- uso total e divisão por collection, incluindo dados e índices;
- catálogo de collections com descrição, política de exportação e política de limpeza;
- exportação de uma collection ou do escopo geral em JSON e, quando solicitado, arquivo ZIP com mídias permitidas;
- limpeza por collection ou geral mediante frase explícita de confirmação;
- trilha de auditoria de início, conclusão, falha ou conclusão parcial da operação.

Durante uma limpeza, um modo de manutenção bloqueia novas mutações concorrentes. Referências e assets são tratados em etapas para evitar arquivos órfãos; coleções críticas possuem políticas mais restritivas. Exports podem conter dados pessoais e devem ser armazenados fora da aplicação com o mesmo nível de proteção aplicado ao banco.
...
- Uma mensagem privada cria ou atualiza a conversa conhecida, mas **não concede por si só consentimento para campanhas**.
- Os comandos dinâmicos `/notify-me` e `/verify-me` registram a autorização explícita e abrem o menu de onboarding; `/stop` revoga e `/help` apresenta os comandos disponíveis.
...
Cada mudança recebida da Meta também é registrada em uma coleção de eventos com tipo, estado de processamento, resumo seguro e payload criptografado. A API lista esses eventos com paginação e filtros e expõe o payload completo somente a administradores autenticados. Eventos que não representam mensagens — inclusive atualizações de conta, status e tipos ainda não suportados para conversa — permanecem consultáveis no painel Webhook em vez de desaparecerem após a atualização em tempo real.

### Alerta opcional por Gmail

Quando `GMAIL_WEBHOOK_NOTIFICATION_EMAIL` está configurado e o Gmail está operacional, uma nova mensagem recebida por Telegram ou WhatsApp pode gerar um aviso administrativo por email. O assunto identifica o canal e o remetente; o envio é reservado de forma idempotente para evitar duplicações de um mesmo evento. A falha desse alerta auxiliar é auditada, mas não invalida o webhook, não bloqueia a conversa e não altera o consentimento do contato.
...
| Gmail | usuário, App Password, remetente, nome e `GMAIL_WEBHOOK_NOTIFICATION_EMAIL` opcional |
| Mídia | `MEDIA_PUBLIC_BASE_URL`, `MEDIA_SIGNING_SECRET`, `MEDIA_RATE_LIMIT_MAX` |
