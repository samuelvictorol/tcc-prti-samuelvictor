...
const inboundWebhookEmail = require('../src/services/inbound-webhook-email.service');

test('Telegram nao enfileira alerta de email para grupo nao vinculado', async (context) => {
  const originals = {
    ...
    notifyInboundEmail: inboundWebhookEmail.notify
  };
  context.after(() => {
    ...
    inboundWebhookEmail.notify = originals.notifyInboundEmail;
  });
  settingsManager.getValue = async (key) => (
    key === 'TELEGRAM_WEBHOOK_SECRET' ? 'webhook-secret-unlinked-group' : null
  );
  groupsManager.findByExternalId = async () => null;
  ...
  let alertCount = 0;
  inboundWebhookEmail.notify = async () => { alertCount += 1; return { queued: true }; };

  const result = await telegramManager.webhook({
    update_id: 2_026_081_399,
    message: {
      message_id: 99,
      date: 1_786_600_000,
      text: 'Mensagem de grupo ainda nao vinculado',
      chat: { id: -100445566, type: 'supergroup', title: 'Grupo desconhecido' },
      from: { id: 445566, first_name: 'Samuel' }
    }
  }, 'webhook-secret-unlinked-group');
