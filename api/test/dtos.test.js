...
test('settings bulk aceita destinatario de avisos de mensagens recebidas somente como email valido', () => {
  const valid = bulkSettingsSchema.safeParse({
    body: { email: { webhookNotificationEmail: 'avisos@example.com' } }
  });
  const invalid = bulkSettingsSchema.safeParse({
    body: { email: { webhookNotificationEmail: 'nao-e-email' } }
  });
  assert.equal(valid.success, true);
  assert.equal(valid.data.body.email.webhookNotificationEmail, 'avisos@example.com');
