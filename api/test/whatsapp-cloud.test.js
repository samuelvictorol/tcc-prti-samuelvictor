...
test('template personalizado usa somente os codigos Meta pt_BR e en no payload', () => {
  for (const languageCode of ['pt_BR', 'en']) {
    const normalized = templatesManager.normalizeTemplateInput({
      name: `Template ${languageCode}`,
      channel: 'whatsapp_cloud',
      whatsappCloudPreset: 'custom',
      externalTemplateName: `campanha_${languageCode.toLowerCase()}`,
      languageCode
    });

    assert.equal(normalized.languageCode, languageCode);
    assert.deepEqual(buildCustomTemplateMessage({
      ...
      type: 'template',
      template: {
        name: `campanha_${languageCode.toLowerCase()}`,
        language: { code: languageCode }
      }
    });
  }

  const legacyEnglish = templatesManager.normalizeTemplateInput({
    name: 'Ingles regional antigo',
    channel: 'whatsapp_cloud',
    whatsappCloudPreset: 'custom',
    externalTemplateName: 'ingles_regional_antigo',
    languageCode: 'en_US'
  });
  assert.equal(legacyEnglish.languageCode, 'en');
  assert.equal(buildCustomTemplateMessage({
    name: legacyEnglish.externalTemplateName,
    languageCode: 'en_US',
    builder: legacyEnglish.payload.builder
  ...
  assert.throws(
    () => templatesManager.normalizeTemplateInput({
      name: 'Idioma nao oferecido',
      channel: 'whatsapp_cloud',
      whatsappCloudPreset: 'custom',
      externalTemplateName: 'idioma_nao_oferecido',
      languageCode: 'en_GB'
    }),
    (error) => error.code === 'WHATSAPP_TEMPLATE_BUILDER_INVALID'
      && error.details?.field === 'languageCode'
      && error.details?.allowed?.join(',') === 'pt_BR,en'
  );
});
