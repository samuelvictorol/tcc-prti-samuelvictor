...
test('destinatario de webhook exige Gmail completo, inclusive quando salvo junto no primeiro PUT', async (context) => {
  const originals = { findOne: Setting.findOne, find: Setting.find, updateOne: Setting.updateOne };
  context.after(() => {
    Setting.findOne = originals.findOne;
    Setting.find = originals.find;
  ...
  await assert.rejects(
    settingsManager.setBulk({ email: { webhookNotificationEmail: 'avisos@example.test' } }, '507f1f77bcf86cd799439011'),
    (error) => error.code === 'WEBHOOK_NOTIFICATION_REQUIRES_GMAIL'
  );
  assert.equal(stored.size, 0);

  const result = await settingsManager.setBulk({
    email: {
      user: 'gmail@example.test',
      appPassword: 'senha-de-app',
      from: 'gmail@example.test',
      webhookNotificationEmail: 'avisos@example.test'
    }
  }, '507f1f77bcf86cd799439011');
  assert.ok(result.updated.includes('GMAIL_WEBHOOK_NOTIFICATION_EMAIL'));
  assert.equal(result.configuration.email.webhookNotificationEmail, 'avisos@example.test');
